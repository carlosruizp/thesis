...
The family of the set of $T$-risks of hypothesis is then $\hypspacef^T = \bigcup_{\hypspace \in \hypspacef} \hypspace^T$. Now we can define
\begin{equation}
    \nonumber
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
for $\bm{\hypf}_\lossf, \bm{\hypf'}_\lossf \in \hypspace_\lossf, \hypspace_\lossf$ as a pseudo-metric in $\hypspacef^T$.
%

Then, a \emph{distribution-driven} pseudo-metric is defined. Consider 


Two concepts that capture some important properties of $\hypspacef$ are defined.
\begin{itemize}
    \item The \emph{sample-driven capacity} $\capacity{\bsetsample}$.
    \item The \emph{distribution-driven capacity} $\capacity{\bsetdist}$.
\end{itemize}
