...
% One-sided convergence implies consistency (sufficient), also necessary
...
% growth function and vc entropy
...
% teorema de Vapnik: Maximum Margin Hyperplane
...
\end{equation}
%
Now we can give the first \acrshort{kkt} theorem.
% KKT saddle point (sufficient condition)
\begin{theorem}[\acrshort{kkt} Saddle Point Theorem]\label{th:kkt_saddlepoint}
...
\begin{equation}\label{eq:kkt_condition}
    \opt{\alpha}_i c_i(\opt{x}) = 0 , \; i=1, \ldots, m .
\end{equation}
Equation~\eqref{eq:kkt_condition} is the \acrshort{kkt} condition. Now, from the second inequality, we have that
\begin{equation}
    \nonumber
    f(\opt{x}) + \sum_{i=1}^m \opt{\alpha}_i c_i(\opt{x}) \leq f(x) + \sum_{i=1}^m \opt{\alpha}_i c_i(x) ,
\end{equation}
and using~\eqref{eq:kkt_condition} and $\opt{\alpha}_i c_i(x) \leq 0$, for all $x \in \vecspace$,
\begin{equation}
    \nonumber
...
This theorem proves that the \acrshort{kkt} saddle-point condition is a sufficient one for $x$ to be a solution of the optimization problem.
...
\begin{theorem}[Necessary \acrshort{kkt} conditions]
...
When the convex objective function and constraints are also differentiable, it is possible to define specific \acrshort{kkt} conditions to characterize the solutions of the optimization problem. To do that we will introduce the concept of dual problem and duality. 
%
First, observe that in the saddle point condition presented in~\eqref{eq:saddle_point}, we want to find a point $(\opt{x}, \opt{\alpha}, \opt{\beta})$ that is minimal in $\vecspace$ and maximal in $\reals_{\geq 0}^q \times \reals^p$. 
%
To do this, we will use a different approach, introducing the concept of duality and the duality theorems. 
We need then to find $\sup_{\alpha, \beta} \inf_{x} \lagr(x, \alpha, \beta)$, which leads to the concept of dual problem.
...
\begin{theorem}[Weak Duality]
    ...
    $$ f(\opt{x}) \leq \Theta(\opt{\fv{\alpha}}, \opt{\fv{\beta}}) .$$
\end{theorem}
\begin{proof}
...
%
We can observe also the connection with Theorem~\ref{th:kkt_saddlepoint}, which we present in the following lemma.
\begin{lemma}
