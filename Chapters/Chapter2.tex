...
\section{Kernels and Implicit Features}\label{sec:ch2_kernels}
...
This kind of results is one of the motivations for using kernels. 
...
\begin{figure}[t!]
    ...
    \\
    \caption[Cubic regression problem.]{Cubic regression problem example and two regression functions: one linear and one cubic.}
    \label{fig:cubicreg}
\end{figure}
%
...
\section{Risk and Regularization}\label{sec:risk_reg}
...
% Classification
In a binary classification problem, if we want to minimize the number of misclassified patterns, we use the \emph{accuracy} loss, which, with the labels $\set{-1, 1}$ for the classes, we can define as
\begin{equation}
    \label{eq:acc_def}
    \lossf(y, f(x)) = \ind_{y \neq f(x)} = 
    \begin{cases}
...
% Observe that, although the formula is the same as the one used for the accuracy loss in classification, here the output space $\Yspace$ is the positive reals, so it has a different meaning. 
...
% Consider that our hypothesis space $\hypspace$ is $L_2(\Xspace)$, that is, the squared-integrable measurable functions $h: \Xspace \to \reals$.
...
% %
% Even if the map $\emprisk$ is continuous, its inverse $(\emprisk)^{-1}$ might not be continuous or even defined. In that cases we have, according to Tikhonov nomenclature, an ill-posed problem.
% %
% Here is where the Operator Inversion Lemma~\citep{riesz2012functional} is useful. This lemma states that, with $\mathcal{M}$ being a compact set and $A$ a continuous map $A: \mathcal{M} \to \reals_{\geq 0}$, then there exists an ``inverse'' map $A^{-1}: A(\mathcal{M}) \to \mathcal{M}$ that is continuous.
...
\subsection{Regularization and Representer Theorem} % 
The empirical risk presented in~\eqref{eq:emprisk_def} can be interpreted as a map from the space of hypothesis $\hypspace = L_2(\Xspace)$ to the non-negative real numbers, namely
$$ \emprisk: \hypspace \to \reals_{\geq 0} .$$
%
...
\section{Optimization}\label{sec:ch2_opt}
%
The training of some learning models involve finding a solution of some optimization problem, e.g. \acrshort{svms} or \acrfull{nns}.
%
...
Typically, when we want to find the minimum of a differentiable function $f: \Xspace \to \reals$ with $\Xspace \subset \reals^\dimx$, we look at its stationary points, i.e. those points where the gradient of $f$ is zero. This idea is generalized in the following theorem~\citet{CS2000}.
% Fermats Theorem
...
\begin{theorem}[Saddle Point Theorem]\label{th:kkt_saddlepoint}
...
Looking back again at the formulation~\eqref{eq:lagr_weakth}, assume that there exist a $\opt{x}$ and $\opt{\fv{\alpha}} \in \reals_{\geq 0}^{m}$ such that 
\begin{equation}
    \nonumber
...
\section{Support Vector Machines}\label{sec:ch2_svms}
...
\begin{figure}[t!]
    ...
    \caption[Linearly separable binary classification problem.]{Example of a linearly separable binary classification problem, where classes are represented with colors blue and red.}
...
\begin{figure}[t!]
    ...
    \caption[Linearly separable problem with three different perfect classification rules.]{Linearly separable problem with three different perfect classification rules.}
...
\begin{figure}[t!]
    ...
    \caption[Linearly separable problem separated by a maximum margin boundary.]{Linearly separable problem separated by a maximum margin boundary. The classification boundary is shown with a solid line. The lines parallel to the boundary that contain the closest points to such boundary are depicted with dashed lines.}
...
%
\begin{table}[t!]
    \caption[Classification of support vectors.]{Classification of the vectors in terms of the value of $\opt{\alpha}_i$. The first three rows illustrate the support vectors, while the last one illustrates those instances that are not support vectors.}
    \label{tab:sv_clas}
    \centering
    \scalebox{1}{
     \begin{tabular}{*{4}{c}}
...
\section{SVM Variants}\label{sec:svm_variants}
...
%Moreover, we have discussed the difference between minimizing the empirical and expected risks functionals. 
