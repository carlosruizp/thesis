...
% Chapter
In this chapter, in Section~\ref{sec:ch3_mtl_theory}, we present the fundamentals of \acrshort{mtl} theory. We introduce the concept of \acrfull{ltl}, related tasks, and review some of the most relevant works in this field. Moreover, we present the \acrfull{lupi} paradigm and connect it with the \acrshort{mtl} framework.
%
In Section~\ref{sec:ch3_mtl_kernelmethods} we define the operator-valued kernels, typically used in \acrfull{mtl}, and show their connection with the kernels defined over the tensor product of spaces, leading to an easier formulation.
%
In Section~\ref{sec:ch3_overview} we present a survey of the most relevant \acrshort{mtl} works, and we also update a taxonomy presented in~\cite{ZhangY17aa} defining three main groups: feature-based, parameter-based and combination-based methods.
Given the importance of \acrfull{nns} and kernel methods, we also present a survey of \acrshort{mtl} strategies for each of them. 
...
\begin{equation*}
    ...
    \end{aligned}.
\end{equation*}
\comm{TODO: Explicar Riesz Representation Theorem (Chapter 2)\\}
...
This last result will be useful for expressing \acrshort{mt} Learning problems as standard or Single-Task ones, as it is shown in the following sections.
...
\begin{lemma}\label{lemma:regproblems_kernel}
    The predictions $\dotp{u_r^*}{\phi(x)}$ of the solutions $u_1^*, \ldots, u_\ntasks^*$ from the \acrshort{mt} risk~\eqref{eq:mtl_kernel_altext_original} can be obtained solving the problem
    \begin{equation}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
That is, we have expressed the \acrshort{mt} problem as a Single-Task problem with the \acrshort{mt} kernel is
\begin{equation}
    \nonumber
...
Using the framework for \acrshort{mtl} with Kernel methods we can choose different regularizations, induced by the matrix $\mymat{E}$, which lead to different \acrshort{mt} approaches. 
...
In this chapter, we covered some of the most relevant works in \acrshort{mtl}. The work of~\citep{baxter2000model} sets the foundation for \acrshort{mtl} and \acrshort{ltl} theory, setting bounds for the excess risk, that is, the difference between the best empirical and best expected errors. These bounds are later extended in several works, from which we highlight the work of~\citet{Ben-DavidS03}, that defines a definition for related tasks and use it to derive new bounds. Other important theoretical motivation for \acrfull{mtl} that we have presented is its connection with the \acrshort{lupi} paradigm, presented in~\citet{VapnikI15a}.
Moreover, we have also reviewed the works in \acrshort{mtl} and categorize them in three main groups: feature-based, parameter-based and combination-based methods.
The feature-based methods are more common in linear models or \acrshort{nns}, the parameter-based ones typically use linear models as well, and the combination-based, which is the less common one, can be used also with kernel methods, and is the one that has a connection with the \acrshort{lupi} paradigm.
Also, as we have seen, the \acrshort{mtl} methods based on \acrshort{nns} focus mainly on finding a shared representation beneficial to all tasks. 