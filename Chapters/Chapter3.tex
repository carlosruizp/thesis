...
\begin{equation}
    ...
    \begin{aligned}
        ...
        &\text{s.t.} & & \sum_{\kappa= 1}^K \mymat{Q}_\kappa = \mymat{I} \text{ with } \mymat{Q}_{\kappa t} \in \set{0, 1} .
    \end{aligned}
\end{equation}
...
Another approximation to clusterized \acrshort{mtl} is provided in~\cite{CrammerM12}, where a two-step procedure is described as follows. Considering that $K$ initial clusters are fixed containing the $T$ tasks, then two steps are repeated.
...
Unlike deep learning models, the kernel features used are not learnable, but are implicitly defined a priori by the kernel function chosen, like Gaussian, polynomial or Matérn kernels. This fact makes it more difficult to use a feature-based \acrshort{mtl} approach with kernel models. 
...
% Combination-Based
    ...
    % Cai and Cherkassky
Other approach that seems better suited for kernel methods are combination-based one, first shown in~\cite{EvgeniouP04}, which can be seen in~\eqref{eq:mtl_combination_primal}, and then extended in~\cite{CaiC09,CaiC12}. The kernelized optimization problem is
\begin{equation}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item multi-reg: This is the case where an $m$-target regression problem is converted into a multi-task problem by replicating $m$ times the features $X$ and using one of the targets for each repetition, so we have $m$ single target regression problems, each considered a different task.
