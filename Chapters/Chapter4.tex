...
These functions, except for the squared one, are not differentiable in their whole domain, but piece-wise differentiable, so we will use the subdifferential when necessary.  That is, let ${\lambda}_{(1)} < \ldots < {\lambda}_{(p)},\; p \leq m$ be those points where $\mathcal{J}$ is not differentiable, clipped to $[0, 1]$ and sorted, we will refer to these points as elbows.
To find the optimal parameters $\lambda_r^*$ we will consider the subdifferential of $\mathcal{J}(\lambda)$ in the elbows, that is, the set $\partial \mathcal{J}(\lambda) = \{c \in \mathbb{R}, \mathcal{J}(z)-\mathcal{J}(\lambda) \leq c (z - \lambda)  \}$ with $z$ in a neighbourhood of $x$.
...
The criterion to define an optimal value $\lambda^* \in [0, 1]$ when using subdifferentials is
\begin{equation}
    \nonumber
...
%
Moreover, a result that is common for all loss functions is~\citet[Proposition 1]{RuizAD21}, which states the following.
\begin{prop}\label{prop:elbows_neurocom2020}
    For the absolute error and hinge losses, $\partial J(\lambda)$ is single valued and constant between the elbows $\lambda_j$.
...
\end{prop}
Using this idea, we can show how to select the optimal parameters $\lambda_r^*$, i.e. those minimizing the training error, with each of the losses considered.
...
\begin{align*}
    ...
    \end{cases} \; .
\end{align*}
That is, the elbows are related to the values $\frac{-d_i}{c_i}$, that can be clipped and sorted to obtain the elbows ${\lambda}_{(1)} < \ldots < \tilde{\lambda}_{(m)}$.
In~\citet[Proposition 2]{RuizAD21} we show the form of the optimal $\lambda^*$.
\begin{prop}[Optimal $\lambda^*$ with hinge loss]\label{prop:hinge_neurocom2020}
    Set first $\lambda_0 = 0$ and $\lambda_{\npertask+1} = 1$. A value $\lambda^*$ is optimal for problem~\eqref{eq:opt_hinge_l1} if and only if $\lambda^* \in \set{0, 1}$ or $\lambda^*$ is an elbow, that is, $\lambda^* = \lambda_{(k)}$ for some $k=1, \dotsc, \npertask$, and
    \begin{equation}\label{sol_hinge}
    Q(k) = -\sum_{j = 1}^{k - 1} \max(0, c_{(j)}) - \sum_{j = k + 1}^{\npertask} \min(0, c_{(j)}) \in \left[\min(0, c_{(k)}), \max(0, c_{(k)}) \right] .
    \end{equation}
    Observe that $Q(k)$ defined for $k=1, \ldots, \npertask$ is a non-decreasing function.
\end{prop}
...
\begin{align*}
    \partial \left[\lambda c + d \right]_+^2 = 
    \begin{cases}
        0 &, \lambda c + d  \leq 0 \\
        2c(\lambda c + d) &, \lambda c + d  > 0 \\
    \end{cases} \; .
\end{align*}
...
In~\citet[Proposition 2]{RuizAD21} we show the form of the optimal $\lambda^*$ when using the squared hinge loss.
\begin{prop}[Optimal $\lambda^*$ with squared hinge loss]\label{prop:sqhinge_neurocom2020}
    For the error of ~\eqref{eq:opt_hinge_l2}, set first $\lambda_0 = 0$ and $\lambda_{\npertask+1} = 1$, and for each $\lambda_k$ value, $0 \leq k \leq \npertask$, define $\widehat{\lambda}_k$ as 
%
\begin{equation}\label{sol_hinge_2}
 \widehat{\lambda}_k = - \frac{\sum_{j = 1}^{k - 1} \max(0, c_{j}) d_{j} + \sum_{j = k + 1}^{\npertask} \min(0, c_{j}) d_{j}}{\sum_{j = 1}^{k - 1} \max(0, c_{j})^2 + \sum_{j = k + 1}^{\npertask} \min(0, c_{j})^2} .
\end{equation}
%
Then, when $\widehat{\lambda}_0 < 0$, problem~\eqref{eq:opt_hinge_l2} has a minimum at $\lambda^* = 0$.
On the other hand, if $\lambda_k \leq \widehat{\lambda}_k \leq  \lambda_{k+1}$ for some $\widehat{\lambda}_k$, then $\lambda^* = \widehat{\lambda}_k$ is a minimum of ~\eqref{eq:opt_hinge_l2}.
Finally, if neither of the previous conditions holds, \eqref{eq:opt_hinge_l2} has a minimum at $\lambda = 1$.
\end{prop}
