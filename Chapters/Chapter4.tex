...
%There is a worldwide energy transition towards renewable sources, with a particular emphasis in wind and solar generation.
%As a consequence, both are having a big impact on electricity markets and on the management of electrical systems.
%This implies, among other things, a great demand of accurate energy forecasts, an area where machine learning (ML) techniques have a growing presence.
%e
There is a worldwide energy transition towards renewable sources, with~a particular emphasis in wind and solar generation,
%As a consequence, both are having a big impact on electricity markets and on the management of electrical systems.
which implies, among~other things, a~great demand by transmission system operators, wind and solar farm managers or market agents, of~accurate energy forecasts to be made at the time horizons of interest for each actor.
These go from the very short (up to one hour), short (up to a few~hours), or medium-long (from one to several days ahead).
In this work, we will deal with a particular case of the latter, namely, the hourly, day-ahead prediction, where tomorrow's energy production at each hour must be predicted today.
%This day ahead prediction is particularly important for system operators to plan and organize next day's energy generation and, in several countries, Spain among them, for market operators, as the energy price for a given day is settled the day before.
%More precisely, in the case of Spain's daily market, energy predictions are to be given at least about fourteen hours in advance, as farm operators and market agents must make available by hour 10 in Spain's legal time their predictions of the energy produced at each hour of the next day.

Machine learning (ML) techniques have a growing presence among the different approaches to renewable energy forecasting.
ML models require first to choose the predictive features to be used, which depends again on the forecasting horizon.
For instance, in~short-term prediction one can use past energy production values and/or real-time meteorological information, if~available; on the other hand, for~medium--long forecasts the features more widely used are those derived from numerical weather predictions (NWPs) provided by entities such as the European Centre for {Medium-Range  Weather Forecasts} (ECMWF; the~ones we will use) or the {National Centers for Environmental Prediction} %
 (NCEP). %Please change it not to be capitalized if unnecessary. %which are given for up to ten days in advance.
%
%The ECMWF offers NWP forecasts twice a day, the first one from  a run that starts at \utc{00} and which is widely available at about \utc{06}, and the second one from a run that starts at \utc{12} and is available at about \utc{18}.
For the day-ahead energy predictions of interest here, we will use the NWP forecasts given in today's ECMWF run at \utc{00} to predict either PV or wind energy production for tomorrow's hours between 00 and \utc{23}.
In other words, we use NWP forecasts from {the} %Please confirm 00UTC. Should it have a space between 00 and UTC?
 \utc{00} run to predict energy production on the horizons between 24 and 47~h.
 
Once the predictive features are selected, the~usual ML approach is to choose the method deemed to be better suited to the problem at hand, carefully adjust the model hyperparameters and finally build, exploit,~and maintain the optimally hyperparametrized model.
In particular, we can either build local models for a single installation or global models for all the installations within a more or less large geographic area.
%
Many ML models have been proposed for renewable energy prediction, and in Section~\ref{literature} we give  a literature review with a particular emphasis on MTL methods.
%Once the predictive features are selected, the usual ML approach is to choose the method deemed better suited to the problem at hand, carefully adjust the model hyper-parameters and finally build, exploit and maintain the optimally hyper-parametrized model.
%This model can incorporate several feature sources, such as past energy production values, real time meteorological information or numerical weather predictions (NWPs); also, we can either build local models for a single installation or global models for all the installations within a more or less large geographic area.
%Many ML models have been proposed for renewable energy prediction.
%In the case of wind energy,~\cite{giebel_soa,pinson2013,colak} are general reviews of the application of ML models while~\cite{heinermann,zhu_genton} propose concrete models for more specific prediction horizons.
%For solar energy there is again a very large literature addressing many different approaches to short, medium and long term solar irradiation and energy forecasting;
%such as physical, engineering, statistical or machine learning (ML) methods; 
%recent surveys are~\cite{Antonanzas2016,Inman2013,Wan2015} and a good comprehensive reference for many aspects of solar energy is~\cite{SEK} (see also Section~\ref{literature} for a literature review).

However, any ML approach has to handle the behavior of either a single wind or solar farm or of a collection of them, behavior that may change substantially according to different time or operating conditions.
This is obviously the case in photovoltaic (PV) energy, which presents markedly hourly effects and where an installation behavior at midday is very different from that at sunrise or sunset.
The same can be said about seasonal effects, with~summers obviously having stronger radiation and more stable atmospheric conditions than winters.
Turning our attention to wind energy, the~effect of individual hours in a farm's production is much less substantial than in the PV case, but~the day--night shift in the boundary layer behavior 
%(rising during the day and sinking at night) 
can have substantial effects on wind farms~\cite{Porte-Agel} %Interaction between Large Wind Farms and the Atmospheric Boundary Layer
and, therefore, be also reflected in the energy produced.
A second example is the effect of wind direction on the output of a wind farm, as~they are usually  built to take advantage of the prevalent wind directions at their sites.
% but if these directions become, say, orthogonal to the prevalent one, wake effects will appear and result in lower energy production.
Finally, the~power curve of wind turbines has three different response zones: a first one for low speed and near zero production, an~intermediate one with power growing with wind velocity and a third one, with~maximum constant power up to a cut-off~speed.
