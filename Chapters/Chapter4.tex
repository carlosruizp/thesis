...
\begin{equation}\nonumber
    \begin{aligned}
        ...
        &\qquad - \sum_{r= 1}^T \lambda_r \sum_{i=1}^{m_r} \alpha_i^r  y_{i}^r \left[ \left(\sum_{s= 1}^T \lambda_r \sum_{j=1}^{m_s} {\alpha_j^s}  y_j^s \phi({x}_j^s)  \right) \cdot \phi({x}_{i}^r) \right]   \\
        &\qquad -  \sum_{r= 1}^T (1-\lambda_r) \sum_{i=1}^{m_r} \alpha_i^r  y_{i}^r \left[  \left((1-\lambda_r) \sum_{j=1}^{m_r} {\alpha_j^r}  y_j^r \phi_r({x}_j^r) \right) \cdot \phi_r({x}_{i}^r)  \right]  \\
...
\begin{equation}\label{eq:convexmtl_nn_commonupdate}
    \begin{aligned}
        ...
        \Theta^{\tau + 1} &\gets \Theta^\tau - \eta \left[ \frac{\partial}{\partial \hat{y}_i^t}  \lossf(\hat{y}_i^t, y_i^t) \vert_{\hat{y}_i^t = h_t(x_i^t)}  \lambda_t  w^\intercal \nabla_\Theta f(x_i^t, \Theta + \mu  \nabla_\Theta \Omega( \Theta)  \right],
    \end{aligned}
\end{equation}
...
\begin{equation}\label{eq:convexmtl_nn_specificupdate}
    \begin{aligned}
        ...
        \Theta_t^{\tau + 1} &\gets \Theta_t^\tau - \eta_t \left[ \frac{\partial}{\partial \hat{y}_i^t}  \lossf(\hat{y}_i^t, y_i^t) \vert_{\hat{y}_i^t = h_t(x_i^t)}  (1 - \lambda_t)  w^\intercal \nabla_{\Theta_t} f(x_i^t, \Theta_t + \mu  \nabla_{\Theta_t} \Omega( \Theta_t) \right],
    \end{aligned}
\end{equation}
