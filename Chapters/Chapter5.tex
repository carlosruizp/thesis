...
More concretely, we can name this definition as the algebraic product tensor product of vector spaces and denote it as $V \otimes_{\text{alg}} W$, which is just $V \otimes W$ in the finite dimensional case.
...
\end{equation}
%
Extending this definition to the infinite-dimensional case, such as the tensor product of Hilbert spaces is not trivial, and we follow roughly the work of~\citet{Kadison1983}. 
...
\begin{equation}\nonumber
    \begin{aligned}
        f \otimes_{\text{alg}} g: &\Xspace_1 &\otimes & &\Xspace_2 &\to &\reals \\
            &x_1 & \otimes & &x_2 &\to & f(x_1) g(x_2) ,
    \end{aligned}
\end{equation}
...
\begin{proposition}
    ...
    \begin{equation}
        ...
        &(x_1 \otimes x_2) & &&(\hat{x}_1 \otimes \hat{x}_2) &\to & K_1(x_1, \hat{x}_1)  & K_2(x_2, \hat{x}_2) & ,
        \end{aligned}
    \end{equation}
    is a reproducing kernel for the Hilbert space $\hilbertspace_1 \otimes \hilbertspace_2$.
\end{proposition} 
\begin{proof}
    First, observe that $(K_1 \otimes K_2)^{x_1 \otimes x_2} =  K_1^{x_1} \otimes K_2^{x_2}  \in \rkhs_1 \otimes \rkhs_2$,
    where 
    \begin{equation}\nonumber
    ...
    \begin{equation}\nonumber
        \dotp{(K_1 \otimes K_2)^{x_1 \otimes x_2}}{f \otimes g} = f(x_1) g(x_2) .
    \end{equation}
    Since the evaluation functionals are bounded as shown in Proposition~\ref{prop:bounded_tensorfun}, by the Riesz theorem, for every $x_1 \otimes x_2 \in \Xspace_1 \otimes \Xspace_2$ there exists a single element $K^{x_1 \otimes x_2} \in \rkhs_1 \otimes \rkhs_2$ such that
    \begin{equation}
        \nonumber
        \dotp{K_{x_1 \otimes x_2}}{f \otimes g}\;  \forall f \otimes g \in \rkhs_1 \otimes \rkhs_2 ,
    \end{equation}
    thus, this element must be $K_{x_1 \otimes x_2} = (K_1 \otimes K_2)^{x_1 \otimes x_2}$.
    %

    Then, we can define 
    \begin{align*}
        (K_1 \otimes K_2)(x_1 \otimes x_2, \hat{x}_1 \otimes \hat{x}_2) &= \dotp{(K_1 \otimes K_2)^{x_1 \otimes x_2}}{(K_1 \otimes K_2)^{\hat{x}_1 \otimes \hat{x}_2}} \\
...
However, in the minimization problem we only have values $z = B e_r = B_r$ for some $r=1, \ldots, T$; then $\dotp{B_r}{B_s} = E^{-1}_{rs}.$
