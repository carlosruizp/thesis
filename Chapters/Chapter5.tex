...
In \acrshort{mtl} a function for each task has to be estimated, and, furthermore, we would like to capture the degree of relation between the tasks as well. Using kernels for these goals imposes some new challenges, which can be tackled from different perspectives.
...
Building upon the ideas discussed in~\cite{EvgeniouP04}, two useful results are presented in~\cite{EvgeniouMP05}, which show how we can apply \acrshort{ctl} methods to \acrshort{mtl} problems.
...
%     \begin{aligned}
%         &R(\myvec{u}) = \sum_{r=1}^{\ntasks} \sum_{i=1}^{\npertask_r} \lossf(y_i^r, \dotp{u_r}{\phi(x_i^r)}) + \mu J(\myvec{u}) ,\\
%     \end{aligned}
% \end{equation}
...
%     \begin{aligned}
%         &S(\myvec{w}) = \sum_{r=1}^{\ntasks} \sum_{i=1}^{\npertask_r} \lossf(y_i^r, \dotp{\myvec{w}}{\mymat{B}_r \phi(x_i^r)}) + \mu \dotp{\myvec{w}}{\myvec{w}} ,\\
%     \end{aligned}
% \end{equation}
...
Now we can define the kernel matrix $\widetilde{Q} = \Phi (E_\otimes)^{-1} \Phi^\intercal$, such that its corresponding kernel function is
\begin{equation}
...
% \begin{equation}
%     \nonumber
%     (w^*, \fv{v}^*) = \sum_{r=1}^\ntasks \sum_{i=1}^{\npertask_r} \alpha_i^r y_i^r (\lambda_r \phi(x_i^r), (1 - \lambda_r) (e_r \otimes \psi({x}_i^r)))
% \end{equation}
...
Between these two extremes of minimal and maximal entropy, there exists an infinite range of matrices whose rows have intermediate entropies. Our goal is then to find an adjacency matrix $\fm{A}$ that reflects the underlying task relations and, therefore, helps to improve the learning process.
