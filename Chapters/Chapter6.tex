...
\begin{itemize}
    \item \fdata{majorca} and \fdata{tenerife}: The goal is to predict the photovoltaic energy production in Majorca and Tenerife, respectively. The tasks are defined as the energy prediction at each of the $14$ hours with sunlight (we remove the night hours). In Section~\ref{sec:convexmlt_renewable} we will describe these problems with more detail.
...
% Results
  ...
  \begin{table*}[t!]
    ...
    \caption[Test regression scores using \acrshort{mae} as the validation metric.]{Test \acrshort{mae} (top) and R2 score (bottom) and Wilcoxon-based ranking. Here, the optimal hyperparameters have been selected using the \acrshort{mae}. The best models are shown in bold.}
  ...
  \begin{table*}[t!]
    %\captionsetup{font=scriptsize}
    \caption[Test regression scores using \acrshort{mse} as the validation metric.]{Test \acrshort{mae} (top) and R2 score (bottom) and Wilcoxon-based ranking. Here, the optimal hyperparameters have been selected using the \acrshort{mse}. The best models are shown in bold.}
...
\begin{table*}[t!]
    %\captionsetup{font=scriptsize}
    \caption[Test classification scores using the F1 score as validation metric.]{Test F1 (top) and accuracy (bottom) scores, global ranking and block-wise Wilcoxon-based rankings for classification problems. The best models in each block are shown in bold.}
...
For the hourly day-ahead predictions of interest here, we use the forecasts of the \acrshort{ecmwf} run at \utc{00} in a given day to predict the hourly energy productions the day after. That is, using the \acrshort{ecmwf} generated at \utc{00}, the energy predictions are given for each hour from {24}h to {47}h.
...
% Many \acrshort{ml} approaches have been proposed for energy forecasting. 
...
Anyway, any \acrshort{ml} approach has to deal with the changing behaviour of a wind or solar farm, which can be altered substantially according to different conditions or time.
...
%
\begin{table}[t!]
    \caption[Hyperparameters, grids used to find them (when appropriate), and hyperparameter selection method for each model.]{Hyperparameters, grids used to find them (when appropriate), and hyperparameter selection method for each model. Here, $d$ is the number of {dimensions} % Please chang the font in the Table.
...
\begin{itemize}
    \item \fmod{CTL-L1}: a \acrshort{ctl} model, that is, a single SVR for all tasks. Its set of hyperparameters is $\set{C, \gamma, \epsilon}$, where $C$ is the regularization trade-off parameter, $\gamma$ is the kernel width, and $\epsilon$ the width of the error insensitive area.
    \item \fmod{ITL-L1}: an \acrshort{itl} model, that is, an independent SVR for each task, each with its hyperparameters: $\set{C_r, \gamma_r, \epsilon_r}$ for $r=1, \ldots, \ntasks$.
    \item \fmod{MTL-L1}: a convex \acrshort{mtl} model, as shown in Section~\ref{sec:convexmlt_kernel}, whose corresponding set of hyperparameters is in principle $\set{C, \epsilon, \gamma, \gamma_1, \ldots, \gamma_\ntasks, \lambda_1, \ldots, \lambda_\ntasks}$, where $\gamma$ is the kernel width of the common part and $\gamma_r$ the one of the $r$-th specific part; also, $\lambda_r$ is the convex combination parameter corresponding to the $r$-th task.
\end{itemize}
%
...
%, the cost of selecting the optimal hyperparameters scales exponentially with their number, so it is not feasible to use a \acrshort{cv} grid search, for example, if we have more than $3$ hyperparameters.
%
...
\begin{table}[t!]
    ...
    \begin{tabular}{lccccccccc}
        ...
        \midrule
        \fmod{CTL-L1}    &  5.265 &  7.265  & (6) & (4) &  59.322 &  112.985  & (6) & (4) &  - \\
        \fmod{(season)\_ITL-L1}   &  5.305 &  7.384  & (7) & (5) &  59.591 &  113.498  & (7) & (4) &  - \\
        \fmod{(season)\_MTL-L1}   &  \fmaxn{4.884} &  \fmaxn{6.740}  & \fmaxn{(1)} & \fmaxn{(1)} &  53.222 &  101.366  & (2) & (3) &  0.4 \\
        \fmod{(hour)\_ITL-L1}   &  5.083 &  7.015  & (4) & (2) &  54.540 &  103.877  & (3) & (3) &  - \\
        \fmod{(hour)\_MTL-L1}   &  4.957 &  6.840  & (2) & (1) &  \fmaxn{52.614} &  \fmaxn{100.208}  & \fmaxn{(1)} & \fmaxn{(1)} &  0.3 \\
        \fmod{(hour, season)\_ITL-L1}   &  5.250 &  7.251  & (5) & (3) &  57.927 &  110.328  & (5) & (4) &  - \\
        \fmod{(hour, season)\_MTL-L1}   &  5.038 &  6.952  & (3) & (2) &  54.601 &  103.992  & (4) & (3) &  0.3 \\
        \bottomrule
    \end{tabular}
    }
 \end{table}
\unskip
 ...
 \begin{table}[t!]
    ...
    \begin{tabular}{lccccccccc}
        ...
        & {{\mwhu}}	& {{\%}} & {rank} & {Wil.} & {{\mwhusq}}	& {{\textpertenthousand}} & {rank} &  {Wil.}\\
        \midrule
        \fmod{CTL-L1}    &  5.786 &  5.373  & (5) & (5) &   88.323 &  76.174  & (5) & (5) &  - \\
        \fmod{(season)\_ITL-L1}   &  5.930 &  5.545  & (6) & (5) &   97.454 &  84.611  & (6) & (6) &  - \\
        \fmod{(season)\_MTL-L1}   &  5.579 &  5.181  & (4) & (2) &   86.227 &  74.366  & (3) & (3) &  0.8 \\
        \fmod{(hour)\_ITL-L1}   &  5.403 &  5.018  & (2) & (2) &   86.686 &  74.762  & (4) & (4) &  - \\
...
\begin{figure}[t!]
 ...
 \caption[Real energy production against prediction made by the best models {in} \fcode{tenerife}.]{\label{fig:tenerife_best_plots} Real energy production against prediction made by the best models {in} \fcode{tenerife}; the perfect prediction  line is shown in orange. The~units of the axis are \mwhu{}.}
\end{figure}
...
%
In the case of \fdata{majorca}, we plot the predictions \fmod{CTL-L1}, \fmodt{hour}{ITL-L1} and \fmodt{season}{MTL-L1} in Figure~\ref{fig:majorca_best_plots}.
...
%
For \fdata{tenerife} we plot the predictions of \fmod{CTL-L1}, \fmodt{hour}{ITL-L1} and \fmodt{hour}{MTL-L1}, which are shown in Figure~\ref{fig:tenerife_best_plots}.
...
\begin{table}[t!]
    \caption[Test \acrshort{mae} test \acrshort{mse} for \fdata{sotavento}, with the corresponding rankings and associated Wilcoxon-based rankings.]{Test \acrshort{mae} (left), {\acrshort{mse}} (center), with the corresponding rankings and associated Wilcoxon-based rankings, and optimal mixing $\lambda^*$ (right) of the \fdata{sotavento} wind energy models considered. The~best model errors are shown in~bold.}
    \centering
...
& {{\mwhu}} & {rank} & {Wil.} & {{\mwhusq}} & {rank} &  {Wil.}\\
    \midrule
    \fmod{CTL-L1}                                           &   \fmaxn{6.132} & \fmaxn{(1)} & \fmaxn{(1)} &    90.228 & (2) & (2) & - \\
    \fmod{(velocity)\_ITL-L1}                              &  6.211 & (7) & (3) &   93.363 & (7) & (3) & - \\
    \fmod{(velocity)\_MTL-L1}                       &   6.208 & (6) & (3) &    93.199 & (6) & (3) & 0 \\
    \fmod{(timeOfDay)\_ITL-L1}                             &  6.283 & (9) & (4) &   93.594 & (9) & (4) & - \\
    \fmod{(timeOfDay)\_MTL-L1}                      &   \fmaxn{6.132} & \fmaxn{(1)} & \fmaxn{(1)} &    90.228 & (2) & (2) & 1 \\
    \fmod{(timeOfDay, velocity)\_ITL-L1}                 &  6.341 & (11) & (4) &   97.250 & (11) & (5) & - \\
    \fmod{(timeOfDay, velocity)\_MTL-L1}          &  6.312 & (10) & (4) &   94.774 & (10) & (4) & 0.4 \\
    \fmod{(timeOfDay, angle)\_ITL-L1}                    &  6.266 & (8) & (4) &   93.517 & (8) & (4) & - \\
...
Unlike the solar energy case, here the \acrshort{ctl} approach seems better suited than using an \acrshort{itl} one. This is also reflected in the selection of $\lambda^*$ values in the models that get best results, which are close to $1$, the \acrshort{ctl} equivalent case. That is, \acrshort{mtl} models such as \fmodt{timeOfDay}{MTL-L1}, \fmodt{timeOfDay, angle}{MTL-L1} and \fmodt{timeOfDay, angle, velocity}{MTL-L1}, where $\lambda^*=1$, obtain the best results and are equivalent to \fmod{CTL-L1}. Also note the cases of \fmodt{angle}{MTL-L1} and \fmodt{angle, velocity}{MTL-L1}, that are second and third best, and where the selected values are $\lambda^*=0.9$ and $\lambda^*=0.7$.
Those models that put the emphasis on the independent parts, like \fmodt{timeOfDay, velocity}{MTL-L1}, and, of course, the \acrshort{itl} approaches, get worse results.
%
%
...
%
The \acrshort{ctl} approach obtains the best results, being the best model in terms of MAE and second best in terms of MSE. Nevertheless, the equivalent \acrshort{mtl} approaches that use $\lambda^*=1$ trivially tie at first place; these are \fmodt{timeOfDay}{MTL-L1}, \fmodt{timeOfDay, angle}{MTL-L1} and \fmodt{timeOfDay, angle, velocity}{MTL-L1}, while the \fmodt{angle}{MTL-L1} gets the second best MAE score.
When the MSE scores are analyzed, the roles are reversed, the \fmodt{angle}{MTL-L1} gets the best result, and the \fmod{CTL-L1} and the equivalents \acrshort{mtl} approaches are second.
...
\begin{figure}[t!]
 ...
 \caption[Real energy production against prediction made by the best models for \fdata{sotavento}.]{\label{fig:stv_best_plots} Real energy production against prediction made by the best models for \fdata{sotavento}; the perfect prediction line is shown in orange. The~units of the axis are percentage points of the total wind energy~installed.}
 \end{figure}
...
\begin{figure}[t!]
    ...
    \caption[Images of the four classification problems used.]{Images of the four classification problems used. Each image has a title indicating the corresponding task. The rows correspond to \fdata{var-MNIST}, \fdata{rot-MNIST}, \fdata{var-FMNIST} and \fdata{rot-FMNIST} (from top to bottom).}
...
\begin{table}[t!]
    \centering
        \caption{Test accuracy with averaged logits.}
...
% TODO
To test our proposal, we consider the \acrshort{ctl}, \acrshort{itl} and \acrshort{mtl} approaches. More specifically, using our nomenclature LX-\acrshort{svm} to refer for either L1, L2 or LS-\acrshort{svm}, we will compare the following:
\begin{itemize}
    ...
    \item \fmod{MTL-LX}: A Convex \acrshort{mtl} model based on the LX-\acrshort{svm}.
    \item \fmod{GLMTL-LX}: A \acrshort{gl} \acrshort{mtl} model based on the LX-\acrshort{svm}, as presented in Section~\ref{sec:graphlap}.
    \item \fmod{GLMTL-LX}: A convex \acrshort{gl} \acrshort{mtl} model based on the LX-\acrshort{svm}, as presented in Section~\ref{sec:convexgl}.
...
% To test our proposal we compare it with four alternative models: \fmod{CTL-L1}, \fmod{ITL-L1}
%  which with our nomenclature:  ;  all based on the Gaussian L1-\acrshort{svm}, which with our nomenclature.
% %
% \begin{itemize}
%     \item {Common Task Learning \acrshort{svm} (\fmod{CTL})}, which considers a single \acrshort{svm} model for all tasks, without making use of the task information.
%     \item {Independent Task learning \acrshort{svm} (\fmod{ITL})}, which fits an independent \acrshort{svm} for each task.
%     \item {Convex Multi-Task learning \acrshort{svm} (\fmod{MTL})}, which considers the convex \acrshort{mtl} approach, described in Section~\ref{sec:convexmlt_kernel}.
%     \item {Graph Laplacian MTL-\acrshort{svm} (\fmod{GLMTL})}, where only the \acrshort{gl} regularization is used to enforce the coupling between the tasks, as presented in Section~\ref{sec:graphlap}; no common part is incorporated.
%     \item {Convex Graph Laplacian MTL-\acrshort{svm} (\fmod{cvxGLMTL})}, where we use a common and task-specific parts, and those specific parts are coupled through a \acrshort{gl} regularization, as presented in Section~\ref{sec:convexgl}.
% \end{itemize}
...
\begin{table*}[t]
    \caption[Test MAE and R2 scores in the regression problems, and test F1 scores in the classification ones.]{In the left, test MAE  (top), and test R2 scores (bottom) in the regression problems. In the right, test  F1 score (top), and accuracy (bottom) in the classification problems.}
    ...
    \midrule
    {\fmod{CTL-L1}} & 5.265 &  5.786 & 2.254 & 0.035 & {41870.820} & {76.723} & 1.483 & 0.039 & 0.078 & 0.001  &  0.106 & 0.016 & 0.868 & 0.002 \\
    {\fmod{ITL-L1}} & 5.119 & 5.341 & 2.779 & 0.134 & {37043.664} & {371.549} & 1.488 & 0.038 & 0.082 & 0.006  & 0.183 & 0.034 & 0.901 & 0.000 \\
    {\fmod{MTL-L1}} & 5.077 & 5.351 &  \fmax{2.228} & \fmax{0.006} & {36848.971} & {242.052} & \fmax{1.466} & \fmax{0.028} & \fmax{0.074} & \fmax{0.003}   & 0.150 & 0.023 & 0.906 & 0.001 \\
    {\fmod{GLMTL-L1}} & 5.291 & 5.840 & 3.070 & 0.391 & {37123.515} & {404.205} & 1.690 & 0.017 & 0.094 & 0.006   & \fmax{0.227} & \fmax{0.042} & 0.896 & 0.003 \\
    {\fmod{cvxGLMTL-L1}} & \fmax{4.917} & \fmax{5.335} &  2.230 & 0.038 & \fmax{36720.854} & \fmax{225.335} & 1.467 & 0.026 & \fmax{0.074} & \fmax{0.003}   & 0.163 & 0.031 & \fmax{0.908} & \fmax{0.001} \\
    \midrule        
        &\fheadmulti{10}{R2} & \fheadmulti{4}{Accuracy}\\
        \midrule
        {\fmod{CTL-L1}} & 0.831 &  0.902 & 0.843 & 0.044 & {0.638} & {0.005} & 0.560 & 0.017 & 0.743 & 0.022    &  0.942 & 0.004 & 0.791 & 0.003   \\
        {\fmod{ITL-L1}} & 0.843 & 0.904 & 0.776 & 0.017 & {0.696} & {0.005} & 0.550 & 0.024 & 0.711 & 0.006  & 0.942 & 0.004 & 0.850 & 0.000 \\
        {\fmod{MTL-L1}} & 0.845 & \fmax{0.907} & 0.850 & 0.045 & {0.700} & {0.003} & \fmax{0.566} & \fmax{0.013} & \fmax{0.755} & \fmax{0.016}  & 0.943 & 0.004 & 0.858 & 0.002 \\
        {\fmod{GLMTL-L1}} & 0.832 & 0.894 &  0.490 & 0.264 & {0.695} & {0.007} & 0.366 & 0.027 & 0.596 & 0.033  & 0.935 & 0.002 & 0.844 & 0.005 \\
        {\fmod{cvxGLMTL-L1}} & \fmax{0.849} & 0.905 & \fmax{0.852} & \fmax{0.046} & \fmax{0.702} & \fmax{0.003} & \fmax{0.566} & \fmax{0.013} & 0.752 & 0.016  & \fmax{0.944} & \fmax{0.004} & \fmax{0.862} & \fmax{0.002} \\
        \bottomrule
        \end{tabular}}
\end{table*}
...
% ([.0-9 ()]+&){2}
\begin{table*}[t]
    \caption[Wilcoxon test $p$-values.]{Top: Wilcoxon $p$-values of absolute errors of a regression model and the one following it in the MAE ranking and similar F1 score $p$ values.
    ...
    \midrule
    \fmod{CTL-L1} & 0.0000 & (3) & 0.0000 & (4)  & \fmaxn{0.2554} & \fmaxn{(1)} & 0.0000 & (4) & 0.0002 & (2) & 0.0000 & (2) & 0.0277 & {(3)}\\
    \fmod{ITL-L1} & 0.8131 & (2) & 0.0035 & (2) & 0.0001 & (2) & 0.0318 & (3) & 0.2546 & (2) & 0.3995 & (2) &	\fmaxn{0.3454} & \fmaxn{(1)}\\
    \fmod{MTL-L1} & 0.0000 & (2) & 0.0000 & (3) & {-} & \fmaxn{(1)} &  0.0000 & (2) &  {-} & \fmaxn{(1)} & {-} & \fmaxn{(1)} & 0.0277 & {(2)} \\
    \fmod{GLMTL-L1} & 0.4183 & (3) & 0.5962 & (4) & 0.0621 & (2) &  0.5658 & (3) &  0.0000 & (3) & 0.0000 & (3) & {-} & \fmaxn{(1)} \\
    \fmod{cvxGLMTL-L1} &  {-} & \fmaxn{(1)} & {-} & \fmaxn{(1)} & \fmaxn{0.4113} & \fmaxn{(1)} &  {-} & \fmaxn{(1)} & \fmaxn{0.0771} & \fmaxn{(1)} & \fmaxn{0.6093} & \fmaxn{(1)} & \fmaxn{0.3454} & \fmaxn{(1)}\\
    \midrule
    &\fheadmulti{12}{R2} & \fheadmulti{2}{Accuracy}\\
        \midrule
    \fmod{CTL-L1} & 0.0032 & (3) & 0.0000 & (2)  & \fmaxn{0.1791} & \fmaxn{(1)} & 0.0000 & (4) & 0.0016 & (3) & 0.0001 & (2) & 0.3454 & (4) \\
        \fmod{ITL-L1} & 0.6340 & (2) & \fmaxn{0.5999} & \fmaxn{(1)} & 0.0001 & (2)  & 0.0035 & (3) & 0.3096 & (3) & 0.3972 & (2) & 0.0277 & (3)\\
        \fmod{MTL-L1} & 0.0000 & (2) & \fmaxn{0.0815} & \fmaxn{(1)} & {-} & \fmaxn{(1)} & 0.0000 & (2) &  {-} & \fmaxn{(1)} & {-} & \fmaxn{(1)} & 0.0431 & (2)\\
        \fmod{GLMTL-L1} & 0.2040 & (3) & 0.7790 & (2) & 0.0384 & (3) &  0.6759 & (3) &  0.0000 & (4) & 0.0000 & (3) & 0.0277 & (4) \\
        \fmod{cvxGLMTL-L1} &  {-} & \fmaxn{(1)} & {-} & \fmaxn{(1)} & \fmaxn{0.2606} & \fmaxn{(1)} &  {-} & \fmaxn{(1)} &  0.0181 & (2) & \fmaxn{0.7262} & \fmaxn{(1)} & 	{-} & \fmaxn{(1)} \\
        \bottomrule
    \end{tabular}}
\end{table*}
...
\begin{table*}[t]
    ...
    &\fheadmulti{10}{MAE} \\
    \midrule
        {\fmod{CTL-L1}}              &       3.440 &        4.183 &   1.557 & 0.198 &  {40502.686} & {222.209} &  1.434 & 0.019 &   0.055 & 0.006 \\
        {\fmod{ITL-L1}}              &       3.590 &        3.914 &   1.883 & 0.224 &   {34403.940} & {83.583} &  1.399 & 0.025 &   0.050 & 0.004 \\
        {\fmod{MTL-L1}}       &       3.649 &        3.921 &   1.522 & 0.248 &  {35061.556} & {118.259} &  1.399 & 0.027 &   0.055 & 0.007 \\
        {\fmod{GLMTL-L1}}     &       \fmaxn{2.630} &        \fmaxn{3.728} &   2.077 & 0.447 &  \fmaxn{33984.568} & \fmaxn{151.998} &  1.594 & 0.023 &   \fmaxn{0.038} & \fmaxn{0.002} \\
        {\fmod{cvxGLMTL-L1}} &       3.344 &        4.141 &   \fmaxn{1.516} &\fmaxn {0.270} &  {34409.942} & {101.472} &  \fmaxn{1.406} &\fmaxn {0.023} &   0.057 & 0.007 \\
        % \midrule
        % &\fheadmulti{10}{R2} \\
        %     \midrule
...
%
To make this comparison we consider the models \fmod{CTL-LX}, \fmod{ITL-LX}, \fmod{MTL-LX} and \fmod{cvxGLMTL-LX} already described, and we also add the adaptive convex \acrshort{gl} model, as presented in Section~~\ref{sec:adapconvexgl}, which we call \fmod{AdapGLMTL-LX}.
%\fmod{AdapGLMTL-LX}: An Adaptive Convex GL \acrshort{mtl} model based on the LX-\acrshort{svm}, as presented in Section~~\ref{sec:adapconvexgl}.
% \acrshort{ctl}, \acrshort{itl} and \acrshort{mtl} approaches. More specifically, using our nomenclature LX-\acrshort{svm} to refer for either L1, L2 or LS-\acrshort{svm}, we will compare the following:
% \begin{itemize}
%     \item \fmod{CTL-LX}: A \acrshort{ctl} model based on the LX-\acrshort{svm}.
%     \item \fmod{ITL-LX}: An \acrshort{itl} model based on the LX-\acrshort{svm}.
%     \item \fmod{MTL-LX}: A Convex \acrshort{mtl} model based on the LX-\acrshort{svm}.
...
\begin{table}[t!]
    \caption[Hyperparameters, grids used to select them (when appropriate) and hyperparameter selection method for each model in the synthetic and real problems.]{Hyperparameters, grids used to select them (when appropriate) and hyperparameter selection method for each model in the synthetic and real problems.
...
\begin{table}
    \caption[\acrshort{mae} scores for synthetic regression problems and F1 scores for synthetic classification ones.]{\acrshort{mae} scores for synthetic regression problems and F1 scores for synthetic classification ones. In bold we highlight the best models of each group: L1, L2 or LS-\acrshort{svms}.}
...
%         \fmod{cvxGLMTL-L1}       &            0.212 &            0.173 &            0.138 \\
...
%         \fmod{cvxGLMTL-L2}     &            0.204 &            0.169 &            0.131 \\
...
%         \fmod{cvxGLMTL-LS}     &            0.207 &            0.169 &            0.132 \\
...
%     \fmod{cvxGLMTL-L1}       &             0.920 &             0.926 &             0.912 \\
...
%     \fmod{cvxGLMTL-L2}     &             0.921 &             0.923 &             \fmaxn{0.915} \\
...
%     \fmod{cvxGLMTL-LS}     &             0.919 &             \fmaxn{0.921} &             0.897 \\
...
Here, observe that we include the values $\mu=0$ and $\nu=0$, which ensures that the the \fmod{cvxGLMTL} model contains the \fmod{MTL} one, with $\nu=0$, and also \fmod{AdapGLMTL} contains \fmod{cvxGLMTL} with an identity matrix (minimal entropy rows) when $\mu=0$.
%
...
%      \begin{tabular}{*{8}{c}}
%      \toprule
%      \fhead{} & \fhead{Grid} & \fhead{\fmod{CTL-LX}} & \fhead{\fmod{ITL-LX}} & \fhead{\fmod{MTL-LX}}  & \fhead{\fmod{cvxGLMTL-LX}} & \fhead{\fmod{AdapGLMTL-LX}}    \\
%      \midrule
%       $C$ &  \scalebox{1}{$\set{10^k: 0 \leq k \leq 4}$} & \checkmark & \checkmark & \checkmark & - & - \\ 
%       $\epsilon$ & \scalebox{1}{$\set{\frac{1}{10^k}: 1 \leq k \leq 4}$} & \checkmark & \checkmark & \checkmark & - & - \\
%       $\gamma$ & \scalebox{1}{$\set{\frac{10^k}{d}: -3 \leq k \leq 2}$} & \checkmark & \checkmark & \checkmark & - & - \\
...
in these variants the non-adaptive \fmod{cvxGLMTL} approach has the best results.
%
...
%
% In fact, when drawing the graph adjacency matrices (not shown for space considerations), they are essentially diagonal for the three real problems; in particular, no clustering between them appears and, therefore, \fmod{AdapGLMTL} cannot draw any advantage on these problems to improve the performance of either \fmod{MTL} or \fmod{cvxGLMTL}.

\begin{table*}[t]
    \caption[Test results for real problems using the adaptive \acrshort*{gl} method.]{Test results for real problems.
...
%      \fmod{cvxGLMTL-L1}     & \fmaxn{1.526} $\pm$ \fmaxn{0.052}  & 1.945 $\pm$ 0.024 & 0.249 $\pm$ 0.039 \\
...
%     \fmod{cvxGLMTL-L2}     & 1.507 $\pm$ 0.045 & \fmaxn{1.927} $\pm$ \fmaxn{0.037} & 0.251 $\pm$ 0.048 \\
...
%     \fmod{cvxGLMTL-LS}     & \fmaxn{1.502} $\pm$ \fmaxn{0.047} & \fmaxn{1.917} $\pm$ \fmaxn{0.022} & \fmaxn{0.197} $\pm$ \fmaxn{0.033} \\
