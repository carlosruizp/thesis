...
%
The \fdata{computer} dataset~\cite{Lenk96}, which has been used in several works~\citep{ArgyriouEP08,AgarwalDG10,KumarD12,JeongJ18}, represents the likelihood of purchasing different computer models, as 



%
% Explicar qué problemas tengo y qué características tienen
The \fdata{computer} dataset~\cite{Lenk96}, used in~\cite{ArgyriouEP08,AgarwalDG10,KumarD12,JeongJ18}, contains the likelihood of purchasing one among 20 computer models as obtained from a survey of 190 people, who value 13 binary attributes of each model.
...
%
The \fdata{parkinson} dataset\footnote{Available at \url{https://archive.ics.uci.edu/ml/datasets/Parkinsons+Telemonitoring}.} has the voice data of 42 people with early stage of Parkinson's disease.
...
%
The \fdata{landmine} dataset contains examples collected from various areas with landmines.
...
More precisely, the cross validation is performed %using a random search of $50$ points 
...
%That is, for each of the $5$ possible training sets, with four internal folds, and validation sets, with one internal fold, we try $50$ random combinations of hyperparameters and compute their validation scores.
...
Since the number of hyperparameters for the \fmod{GLMTL} and \fmod{AdapGLMTL} models that include Laplacian regularization is too large, and the size of the hyperparameter space scales exponentially, we first pre-select the $C, \epsilon, \gamma, \gamma_s$ hyperparameters as the optimal ones for the \fmod{MTL} model.
...
Notice that the inclusion of the $\mu = 0$ and $\nu = 0$ values on the hyperparameter ranges ensures that \fmod{GLMTL} contains \fmod{MTL} (when $\nu = 0$) and that \fmod{AdapGLMTL} contains \fmod{GLMTL} (when $\mu = 0$).
...
In \fdata{computer} our \fmod{AdapGLMTL} proposal gives different results for each LX variant. With the L2 variant, the adaptive approach obtains the best result. However, in the L1 and LS variants, while getting the best validation scores, \fmod{AdapGLMTL} does not seem to generalize well in the test set, where the non-adaptive \fmod{GLMTL} model obtains better results.
Summing this up, in \fdata{computer} \fmod{AdapGLMTL} is best overall for the L2 setting, essentially ties for second place with \fmod{MTL} for LS, and is third for L1.
In \fdata{parkinson} \fmod{AdapGLMTL} is very close to the best models for L1 and L2, and is the second best for LS.
%the results are also close to those of \fmod{MTL}.
% Our proposal is worse for the L1 variant, ties for the L2 one and is better for the LS variant.
% \comm{CA: No entiendo las dos frase anteriores, nuestra propuesta es \fmod{AdapGLMTL}, ¿no? Hay algo que no cuadra...}
%In the \fdata{landmine} problem, which is a classification one, the results are similar. 
In the \fdata{landmine}, a classification problem, \fmod{AdapGLMTL} is best for L1, essentially ties with \fmod{GLMTL} for the first place for LS, but is fourth for L2.
%\comm{That is, the rankings are not consistent between variants: our proposal obtains clearly the best result for the L1 variant, but it is clearly worse than the independent approach for the L2 variant and ties for the LS one with the non-adaptive \fmod{GLMTL}.}
%The similarity between the GL models and the \fmod{MTL} can be found in the experimental procedure used, since most hyperparameters used in the GL models are taken from the \fmod{MTL} approach.
In any case, we point out that the multi-task models obtain the best results in all cases except for the L2 variant in \fdata{landmine}. Also, the GL approaches either tie or surpass the convex \fmod{MTL} approach in every problem and our proposal, \fmod{AdapGLMTL}, obtains the best score in two occasions and is quite competitive in almost all others.

We also observe that the difference in the performance of \fmod{AdapGLMTL} between synthetic and real problems is remarkable.
However, there is an explanation for this behavior. Our \fmod{AdapGLMTL} approach relies on pushing together those tasks whose models are already close. In the synthetic problems, there are well-defined clusters of tasks and this strategy leads to a smart grouping and better results. In real problems, the situation might be different. The relation among tasks is not clear and the information inferred from the training set might not be that relevant. 
...
%\comm{negativo? quitar? Moreover, the training of the \fmod{AdapGLMTL} algorithm includes more overhead than that of the \fmod{GLMTL}, since there are multiple iterations and matrix updating steps. This complexity might make it more challenging to efficiently exploit all its properties in real world problems.}
