...
\begin{frame}
      ...
      \note[item]{Intentar darle misterio/interés al AA}
...
\begin{frame}
      {Riesgo esperado}
      \begin{itemize}
            \item Definimos el Riesgo Esperado como
            \begin{myequation}
                  \nonumber
            ...
            \begin{myequation}
              ...
              \end{myequation}
            \item Pero la distribución $\distf(x, y)$ es desconocida
      \end{itemize}
...
\begin{frame}
      ...
      \begin{itemize}
            ...
            \begin{myequation}
              ...
              \end{myequation}
            \item Definimos el Riesgo Empírico como
            \begin{myequation}
                  \nonumber
...
\begin{frame}
      \frametitle{Riesgo empírico multitarea}
...
\begin{frame}
      ...
      \begin{itemize}
            ...
            \begin{itemize}
                  ...
                  \begin{myequation}
                        \nonumber
                        \argmin_{\fv{\hypf} \in \hypspace^\ntasks} \left\{ \sum_{r=1}^\ntasks \frac{1}{\npertask_r} \sum_{i=1}^{m_r} \ell(\hypf_r(x_i^r), y_i^r) +  \sum_{r=1}^\ntasks \Omega_r(h_r) \right\}
                  \end{myequation}
...
\begin{frame}
      ...
      \includegraphics[width=.8\textwidth]{Figures/schools_standard.png}
...
\begin{frame}
      ...
      \note[item]{Enfatizar los dos mundos: kernel y neuronal}
...
\begin{frame}
      ...
      \begin{columns}
            \begin{column}{0.5\textwidth}
                  \begin{itemize}
                        %\item  Las Máquinas de Vectores Soporte (SVM) son modelos populares de AA
...
\end{frame}
  ...
  \begin{frame}
      ...
      \begin{columns}
            \begin{column}{0.5\textwidth}
                  \begin{itemize}
                        %\item  Las Redes Neuronales (NN) tienen una gran popularidad
            ...
            \begin{column}{0.5\textwidth}  %%<--- here
                \begin{center}
                  \scalebox{.75}{
                  \input{Diagrams/standard_nn_image.tex}
                  }
                 \end{center}
            \end{column}
      \end{columns}
...
\begin{itemize}
      ...
      \begin{itemize}
            \item %{Combinación convexa para SVM MT}
            Formulación más interpretable para el aprendizaje MT con SVM
            \item %{Combinación convexa para redes neuronales MT}
            Modelos neuronales MT alternativos basados en combinaciones
            \item %{Regularización laplaciana para SVM MT}
            Método para aprender la relación entre tareas %con SVM
            
      \end{itemize}
      \item \textbf{Contribuciones}
      \note[item]{La revisión del estado del arte se muestra en el capítulo 3, y tenemos un artículo en revisión en el que revisamos los trabajos de MTL con SVM}
      \note[item]{Reforzar de nuevo la idea de los dos mundos: kernel para datasets pequeños y neuronal para grandes}
      \begin{itemize}
...
\begin{frame}
    ...
    \begin{itemize}
      ...
      %   \end{itemize}
        \item Fue propuesta~\footfullcite{EvgeniouP04} incialmente para SVM lineales:
        \begin{myequation}
            \nonumber
            h_r(\cdot) = \dotp{w + v_r}{\cdot} + b_r
        \end{myequation}
        \item Fue extendida al caso no lineal~\footfullcite{CaiC09}:
        \note[item]{Cada parte puede tener una transformación distinta, correspondientes a distintos espacios de kernel. + flexibilidad y + hiperpar.}
        \begin{myequation}
            \nonumber
            h_r(\cdot) = \dotp{w}{\phi(\cdot)} + \dotp{{v}_r}{\phi_r(\cdot)} + b_r
      \end{myequation}
      %\item $w$ es el parámetro de la parte común y $v_r$ los de las partes específicas
      % \begin{itemize}
      %       \item 
...
%       \begin{itemize}
%             \item La formulación aditiva de~\footfullcite{CaiC09} con SVM es:
%             \begin{myequation}
%                   \nonumber
...
%   \begin{frame}
%         \frametitle{Formulación Aditiva para MTL SVM}
    
%         \begin{block}{Problema Dual - SVM Aditiva}
%               \begin{myequation}\nonumber
%                     \begin{aligned}
...
%       \begin{proposition}[Equivalencia entre formulaciones para SVM MT]
...
\note[item]{Son cuatro problemas de regresión lineal y mostramos los parámetros}
%     \begin{itemize}
%       \item Consideramos modelos MT con un $\lambda$ común
%       \item Usamos cuatro problemas de regresión lineal con distintas pendientes
%     \end{itemize}
...
\begin{frame}
      ...
      \note[item]{Decir que no es fácil encontrar problemas MTL}
      \begin{itemize}
            \item Como comprobación, consideramos~\footfullcite{RuizAD21} la combinación convexa de modelos preentrenados
            \begin{itemize}
...
%   \begin{frame}
%       \frametitle{Experimentos: hiperparametrización}
...
%           \item Menos en \fdata{majorca} y \fdata{tenerife}, usamos una validación cruzada anidada con 3 particiones externas e internas estratificadas por tareas
%           \item Los hiperparámetros se eligen con una búsqueda en rejilla con las particiones de entrenamiento y validación
%       %     \item Por limitaciones computacionales nos restringimos a la búsqueda de tres hiperparámetros para la búsqueda en rejilla (reusamos anchuras de kernel)
...
%   \end{frame}
  ...
  \begin{frame}
      ...
      \begin{itemize}
          \item Hacemos una búsqueda en rejilla con una validación cruzada
          \item Usamos tres particiones estratificadas por tareas
  ...
  \begin{frame}{Experimentos: resultados de regresión}
      ...
      %     \fheadmulti{6}{MAE}
      %     \midrule
          & \fhead{\fdata{maj.}} & \fhead{\fdata{ten.}} & \fhead{\fdata{boston}} & \fhead{\fdata{california}} &  \fhead{\fdata{abalone}} & \fhead{\fdata{crime}}\\
          \midrule
          \fmod{ITL-L1}            &  {5.087}  &  {5.743}  &  {2.341} &  {36883.582}  &  {1.481}   &  {0.078}   \\
          \fmod{CTL-L1}            &  {5.175}  &  {5.891}  &  \fmaxn{2.192}   &  {41754.337}  &  {1.482} & {0.078}   \\
          \fmod{CMB-L1} &  \fmaxn{5.047}  &  \fmaxn{5.340} &  {2.239}  &  {36880.238}  &  {1.470}  &  {0.077}  \\
          \fmod{MTL-L1}     &  {5.050}  &  {5.535}  &  {2.206}  &  \fmaxn{36711.383}  &  \fmaxn{1.454} &  \fmaxn{0.074}  \\
          \midrule
          \fmod{ITL-L2}            &  {4.952}  &  \fmaxn{5.629}  &  {2.356}  &  {37374.618}  &  {1.498}  &  {0.079} \\
          \fmod{CTL-L2}            &  {5.193}  &  {6.107}  &  \fmaxn{2.083}  &  {42335.612} &  {1.503}  &  {0.080} \\
          \fmod{CMB-L2} &  {4.869}  &  {5.963}  &  {2.089}  &  {37374.618} &  {1.494}  &  {0.077}  \\
          \fmod{MTL-L2}     &  \fmaxn{4.854}  &  {5.784}  &  {2.089} &  \fmaxn{37202.603} &  \fmaxn{1.482}  &  \fmaxn{0.077} \\
          \midrule
          \fmod{ITL-LS}            &  {4.937}  &  {5.649}  &  {2.204}  &  {37348.347}  &  {1.496}  &  {0.079}  \\
          \fmod{CTL-LS}            &  {5.193}  &  {6.005}  &  \fmaxn{2.072}  &  {42259.492}  &  {1.502}  &  {0.079}  \\
          \fmod{CMB-LS} &  {4.977}  &  \fmaxn{5.593}  &  {2.081} &  {37339.179}  &  {1.486}  &  {0.079}  \\
          \fmod{MTL-LS}     &  \fmaxn{4.824}  &  {5.754}  &  {2.077} &  \fmaxn{37231.043}  &  \fmaxn{1.478}  &  \fmaxn{0.076}  \\
          \bottomrule
          \end{tabular}}
        \end{table}
      ...
      \begin{table}[t]
          ...
          \scalebox{.7}{
            \begin{tabular}{ l*{8}{S[table-format=1.3]}}
              \toprule
            %   \fheadmulti{8}{Score F1}
            %   \midrule
              & \fhead{\fdata{comp\_(G)}} & \fhead{\fdata{comp\_(R)}} & \fhead{\fdata{comp\_(G,R)}} & \fhead{\fdata{ad\_(G)}} & \fhead{\fdata{ad\_(R)}} & \fhead{\fdata{ad\_(G,R)}} & \fhead{\fdata{landmine}} & \fhead{\fdata{binding}} \\
              \midrule
              \fmod{ITL-L1}    &          {0.625} &           \fmaxn{0.639} &                  {0.630} &         \fmaxn{0.659} &          {0.653} &                 {0.657} &    {0.231} &   {0.867} \\
              \fmod{CTL-L1}    &          {0.623} &           {0.638} &                  {0.638} &         {0.657} &          {0.650} &                 {0.653} &    {0.255} &   {0.901} \\
              \fmod{CMB-L1} &          {0.616} &           {0.638} &                  {0.638} &         {0.658} &          {0.650} &                 {0.653} &    \fmaxn{0.270} &   {0.901} \\
              \fmod{MTL-L1}    &          \fmaxn{0.627} &           {0.636} &                  \fmaxn{0.640} &         \fmaxn{0.659} &          \fmaxn{0.655} &                 \fmaxn{0.659} &    {0.242} &   \fmaxn{0.907} \\
              \midrule
              \fmod{ITL-L2}    &          {0.636} &           {0.623} &                  {0.607} &         \fmaxn{0.668} &          \fmaxn{0.666} &                 \fmaxn{0.668} &    {0.256} &   {0.867} \\
              \fmod{CTL-L2}    &          \fmaxn{0.640} &           {0.647} &                  \fmaxn{0.651} &         {0.665} &          {0.661} &                 {0.659} &    \fmaxn{0.270} &   {0.903} \\
              \fmod{CMB-L2} &          {0.629} &           {0.640} &                  {0.645} &         {0.666} &          {0.662} &                 {0.661} &    \fmaxn{0.270} &   {0.903} \\
              \fmod{MTL-L2}    &          {0.634} &           \fmaxn{0.651} &                  {0.650} &         \fmaxn{0.668} &          \fmaxn{0.666} &                 \fmaxn{0.668} &    {0.263} &   \fmaxn{0.909} \\
              \midrule
...
\begin{frame}
      \frametitle{Experimentos: predicción de energía solar}
...
\end{frame}
  ...
  \begin{frame}
      \frametitle{Experimentos: predicción de energía eólica}
...
\begin{frame}
      ...
      \note[item]{Hacer énfasis en juntar los dos mundos: kernel y neuronal}
      \begin{itemize}
...
\begin{frame}
      ...
      \begin{itemize}
            \item Proponemos~\footfullcite{RuizAD22_hais} la formulación convexa para redes neuronales MT
            %, combinando:
            % \begin{itemize}
...
\begin{frame}{Ejemplo de formulación convexa para dos tareas}
      ...
      \note[item]{Decir que $\lambda_r$ son hiperparámetros pero pueden ser parámetros}
...
%       \note[item]{Decir que no usamos SVM por su limitación computacional}
%       \begin{itemize}
...
%             \item La aplicamos a:
%             \begin{itemize}
%                   \item SVM MT convexa con regularización laplaciana
      ...
      \begin{block}{Problema primal - SVM convexa con laplaciano}
            \begin{myequation}\nonumber
                  \begin{aligned}
                       ...
                       &                                             & { C \sum_{r=1}^\ntasks \sum_{i=1}^{\npertask_r} {\xi_i^r}  + \frac{\nu}{2} \sum_{r=1}^\ntasks \sum_{s=1}^T L_{rs} \dotp{v_r}{v_s} + \frac{1}{2} \sum_r \norm{{v}_r}^2 + \frac{1}{2} \norm{{w}}^2}                                                                              \\
                       & \text{s.t.}
...
\begin{frame}
      ...
      \begin{itemize}
            ...
            \item Proponemos~\footfullcite{RuizAD21_hais, RuizAD22_igpl} un método para la selección automática de $A$, iterando los pasos
...
% \begin{frame}
%       \frametitle{Experimentos: \fdata{clusters0}}
...
\begin{frame}
      \frametitle{Experimentos sintéticos: resultados regresión}
...
\begin{frame}
      \frametitle{Experimentos: resultados clasificación}
...
%             \begin{tabular}{*{7}{c}}
%             \toprule
%             \fhead{} & \fhead{Grid} & \fhead{\fmod{CTL}} & \fhead{\fmod{ITL}} & \fhead{\fmod{MTL}} & \fhead{\fmod{GLMTL}} & \fhead{\fmod{GLMTL}} \\
%              \midrule
...
\begin{itemize}
      ...
      %\vfill
      \item Proponemos una formulación convexa para aprendizaje multitarea%de una parte común a todas las tareas y otra específica
      \begin{itemize}
            \item SVM
            \item Redes Neuronales
      \end{itemize}
      \item Añadimos una regularización laplaciana para SVM%que regula las relaciones entre cada par de tareas
...
\begin{frame}
      ...
      \begin{itemize}
            ...
            %\item Analizar los hiperparámetros $\lambda_r$ para interpretabilidad
...
\section{Apéndice A: Combinación Convexa de Modelos Preentrenados}
\begin{frame}
...
\begin{frame}
      ...
      \begin{itemize}
            % \item La regularización Laplaciana penaliza las distancias
            % \item ¿Cómo calcular las distancias en un Espacio de Hilbert con Kernel Reproductor?
            \item Inspirados por trabajos previos~\footfullcite{EvgeniouMP05} proponemos lo siguiente:
            \begin{itemize}
            \item Consideramos el problema de minimización (con $E$ una matriz def. pos.)
            \begin{myequation}
                  \label{eq:mtl_kernel_altext_original}
