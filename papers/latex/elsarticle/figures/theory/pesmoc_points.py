...
# This functions finds the global optimum of each objective, which could be seful to
...
def generate_plot_illustration_of_the_process():
...
#	vals2 = np.cos(np.sum(inputs2,axis=1)*7.0)*np.sum(inputs2,axis=1)
	...
	initial_ls = np.array([1.])
    
	inputs1  = np.array([[ 0.2, 0.4, 0.5, 0.6, 0.805 ]]).T
	inputs2  = np.array([[ 0.2, 0.4, 0.5, 0.6, 0.805 ]]).T
	inputs3  = np.array([[ 0.2, 0.4, 0.5, 0.6, 0.805 ]]).T

#	vals1 = np.array([ 0.0, 0.9, 1.2, -1.4, -0.8 ])
#	vals2 = np.array([ -0.25, -0.8, -0.7, 0.9, 0.0 ])

	vals1 = np.sin(inputs1 * 10).flatten()
	vals2 = np.cos(inputs2 * 10).flatten()
	vals3 = np.sin(inputs3 * 10).flatten() * np.cos(inputs3 * 10).flatten()

	objective1 = GP(D, kernel='SquaredExp', likelihood='noiseless', mcmc_iters = n_mcmc_iters, initial_ls = initial_ls)
	objective2 = GP(D, kernel='SquaredExp', likelihood='noiseless', mcmc_iters = n_mcmc_iters)
	constraint1 = GP(D, kernel='SquaredExp', likelihood='noiseless', mcmc_iters = n_mcmc_iters)

#	objective1 = GP(D, kernel='Matern52', likelihood='noiseless')
#	objective2 = GP(D, kernel='Matern52', likelihood='noiseless')

#	objective1 = GP(D, kernel='Matern52', likelihood='gaussian')
#	objective2 = GP(D, kernel='Matern52', likelihood='gaussian')

	objective1.fit(inputs1, vals1, fit_hypers = True)
	objective2.fit(inputs2, vals2, fit_hypers = True)
	constraint1.fit(inputs3, vals3, fit_hypers = True)
	...
	constraints_dict['c1'] = constraint1
        ...
        spacing = np.linspace(0, 1.1, 100)[:,None]

	mean1, var1 = objective1.function_over_hypers(objective1.predict, spacing)
	mean2, var2 = objective2.function_over_hypers(objective2.predict, spacing)
	mean3, var3 = constraint1.function_over_hypers(constraint1.predict, spacing)

	objectives_dict['f1'].set_state(8)
	objectives_dict['f2'].set_state(8)
	constraints_dict['c1'].set_state(8)
	...
	gp_samples['objectives'] = [ sample_gp_with_random_features(objective_gp, NUM_RANDOM_FEATURES) \
		for objective_gp in objectives_dict.values() ]
	funs = gp_samples['objectives']
	gp_samples['constraints'] = [ sample_gp_with_random_features(constraint_gp, NUM_RANDOM_FEATURES) \
		for constraint_gp in constraints_dict.values() ]
	funs = gp_samples['objectives']
	cons = gp_samples['constraints']

	grid = np.linspace(0,1.1,100)[:,None]

	for i in range(inputs1.shape[ 0 ]):
		grid = np.append(grid, np.linspace(inputs1[ i ] - 1e-5, inputs1[ i ] + 1e-5, 50))

	for i in range(inputs2.shape[ 0 ]):
		grid = np.append(grid, np.linspace(inputs2[ i ] - 1e-5, inputs2[ i ] + 1e-5, 50))

	for i in range(inputs3.shape[ 0 ]):
		grid = np.append(grid, np.linspace(inputs3[ i ] - 1e-5, inputs3[ i ] + 1e-5, 50))

	grid = np.sort(grid)
	grid = grid.reshape((len(grid), 1))

	grid_pareto = np.linspace(0,1.1,1000)[:,None]

	val_f1_pareto = funs[ 0 ](grid_pareto, False)
	val_f2_pareto = funs[ 1 ](grid_pareto, False)
	val_c1_pareto = cons[ 0 ](grid_pareto, False)

	val_f1 = funs[ 0 ](grid, False)
	val_f2 = funs[ 1 ](grid, False)
	val_c1 = cons[ 0 ](grid, False)

	values = np.vstack((val_f1_pareto, val_f2_pareto)).T
	all_values = values
	selection = _cull_algorithm(values)
	feasible_selection = np.logical_and(selection, val_c1_pareto>0)
	pareto_set_locations = grid_pareto[ feasible_selection, : ]
	front = values[ feasible_selection, : ]

	print '\tPareto Set size Before Summary:%f' % (float(pareto_set_locations.shape[ 0 ]))

	result = _compute_pareto_front_and_set_summary_y_space(front, pareto_set_locations, 50)
	...
	def GPanimation(d, n):
		...
		s = np.linspace(0, 2 * 3.1415926, n + 1)
		s = s[ 0 : n ]
		t = np.outer(t, s)
		X = r * exp_map(x, t)
		return(X)
	...
	def exp_map(mu, E):
		D = E.shape[ 0 ]
		C = E.shape[ 1 ]
		theta = np.sqrt(np.sum(E**2, 0))
		M = np.outer(mu, np.cos(theta)) + E * np.tile((np.sin(theta) / theta), D).reshape((D, C))

		if np.any(theta < 1e-7):
			M[ : , (theta < 1e-7) ] = mu.reshape((D, 1))

		return(M)

	# We compute the average Covariance matrix at the spacing points for each GP

	samples_orbit1 = GPanimation(spacing.shape[ 0 ], 60)
	samples_orbit2 = GPanimation(spacing.shape[ 0 ], 60)
	samples_orbit3 = GPanimation(spacing.shape[ 0 ], 60)
	samples_orbit4 = GPanimation(spacing.shape[ 0 ], 60)
	samples_orbit5 = GPanimation(spacing.shape[ 0 ], 60)

	C1 = np.zeros((spacing.shape[ 0 ], spacing.shape[ 0 ]))
	C2 = np.zeros((spacing.shape[ 0 ], spacing.shape[ 0 ]))
	C3 = np.zeros((spacing.shape[ 0 ], spacing.shape[ 0 ]))

	for i in range(n_mcmc_iters):
		objectives_dict['f1'].set_state(i)
		objectives_dict['f2'].set_state(i)
		constraints_dict['c1'].set_state(i)
		
		C1 += objective1.predict(spacing, full_cov = True)[ 1 ]
		C2 += objective2.predict(spacing, full_cov = True)[ 1 ]
		C3 += constraint1.predict(spacing, full_cov = True)[ 1 ]

	C1 /= n_mcmc_iters
	C2 /= n_mcmc_iters
	C3 /= n_mcmc_iters

	L1 = spla.cholesky(C1, lower=False).T
	L2 = spla.cholesky(C2, lower=False).T
	L3 = spla.cholesky(C3, lower=False).T

#	for image in range(60):

        fig = plt.figure(figsize=(14,16))
	plt.ylim([ -1.4, 1.5 ])
	plt.plot(inputs1, vals1, color='y', marker='o', markersize=20, linestyle='None')
	plt.plot(spacing, mean1, 'y', linestyle='-', linewidth = 5.0)
	plt.plot(spacing, mean1 + np.sqrt(var1), 'y:', linewidth = 4.0)
	plt.plot(spacing, mean1 - np.sqrt(var1), 'y:', linewidth = 4.0)

	scale = np.linspace(0, 1.0, 30)

	for j in range(30 - 1):
		plt.fill_between(spacing.flatten(), (mean1 - np.sqrt(var1) * scale[ j ] + 1e-3), (mean1 - np.sqrt(var1) * scale[ j + 1 ]), 
			linewidth = 0.0, color = (1.0, 1.0, 0.0, 0.5 * (1.0 - scale[ j ])))
		plt.fill_between(spacing.flatten(), (mean1 + np.sqrt(var1) * scale[ j ] - 1e-3), (mean1 + np.sqrt(var1) * scale[ j + 1 ]), 
			linewidth = 0.0, color = (1.0, 1.0, 0.0, 0.5 * (1.0 - scale[ j ])))

#	plt.plot(spacing, mean1 + np.dot(L1, samples_orbit2[ :, image ]), color = 'r', linestyle='-.', linewidth = 5.0)
#	plt.plot(spacing, mean1 + np.dot(L1, samples_orbit3[ :, image ]), color = 'r', linestyle='-.', linewidth = 5.0)
#	plt.plot(spacing, mean1 + np.dot(L1, samples_orbit5[ :, image ]), color = 'r', linestyle='-.', linewidth = 5.0)

	plt.plot(inputs1, vals1, color='y', marker='o', markersize=20, linestyle='None')
#	plt.savefig('/tmp/results/pred1_' + str(image) + '.pdf', format='pdf', dpi=300, bbox_inches='tight')
#	plt.savefig('/tmp/results/pred1_' + str(image) + '.png', format='png', dpi=100, bbox_inches='tight')
	plt.savefig('pred1.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.close(fig)

        fig = plt.figure(figsize=(14,16))
	plt.ylim([ -1.5, 1.5 ])
	plt.plot(inputs2, vals2, color='g', marker='o', markersize=20, linestyle='None')
	plt.plot(spacing, mean2, 'g', linestyle='-', linewidth = 5.0)
	plt.plot(spacing, mean2 + np.sqrt(var2), color = 'g', linestyle = ':', linewidth = 4.0)
	plt.plot(spacing, mean2 - np.sqrt(var2), color = 'g', linestyle = ':', linewidth = 4.0)

	scale = np.linspace(0, 1.0, 30)

	for j in range(30 - 1):
		plt.fill_between(spacing.flatten(), (mean2 - np.sqrt(var2) * scale[ j ] + 1e-3), (mean2 - np.sqrt(var2) * scale[ j + 1 ]), 
			linewidth = 0.0, color = (0.5, 1.0, 0.5, 0.5 * (1.0 - scale[ j ])))
		plt.fill_between(spacing.flatten(), (mean2 + np.sqrt(var2) * scale[ j ] - 1e-3), (mean2 + np.sqrt(var2) * scale[ j + 1 ]), 
			linewidth = 0.0, color = (0.5, 1.0, 0.5, 0.5 * (1.0 - scale[ j ])))

#	plt.plot(spacing, mean2 + np.dot(L2, samples_orbit2[ :, image ]), color = 'g', linestyle='-.', linewidth = 5.0)
#	plt.plot(spacing, mean2 + np.dot(L2, samples_orbit3[ :, image ]), color = 'g', linestyle='-.', linewidth = 5.0)
#	plt.plot(spacing, mean2 + np.dot(L2, samples_orbit5[ :, image ]), color = 'g', linestyle='-.', linewidth = 5.0)

        plt.plot(inputs2, vals2, color='g', marker='o', markersize=20, linestyle='None')
#	plt.savefig('/tmp/results/pred2_' + str(image) + '.pdf', format='pdf', dpi=300, bbox_inches='tight')
#	plt.savefig('/tmp/results/pred2_' + str(image) + '.png', format='png', dpi=100, bbox_inches='tight')
	plt.savefig('pred2.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.close(fig)

	fig = plt.figure(figsize=(14,16))
	plt.ylim([ -1.4, 1.5 ])
	plt.plot(inputs3, vals3, color='r', marker='o', markersize=20, linestyle='None')
	plt.plot(spacing, mean3, 'r', linestyle='-', linewidth = 5.0)
	plt.plot(spacing, mean3 + np.sqrt(var3), 'r:', linewidth = 4.0)
	plt.plot(spacing, mean3 - np.sqrt(var3), 'r:', linewidth = 4.0)

	scale = np.linspace(0, 1.0, 30)

	for j in range(30 - 1):
		plt.fill_between(spacing.flatten(), (mean3 - np.sqrt(var3) * scale[ j ] + 1e-3), (mean3 - np.sqrt(var3) * scale[ j + 1 ]), 
			linewidth = 0.0, color = (1.0, 0.5, 0.5, 0.5 * (1.0 - scale[ j ])))
		plt.fill_between(spacing.flatten(), (mean3 + np.sqrt(var3) * scale[ j ] - 1e-3), (mean3 + np.sqrt(var3) * scale[ j + 1 ]), 
			linewidth = 0.0, color = (1.0, 0.5, 0.5, 0.5 * (1.0 - scale[ j ])))
...
#	plt.savefig('/tmp/results/pred1_' + str(image) + '.pdf', format='pdf', dpi=300, bbox_inches='tight')
#	plt.savefig('/tmp/results/pred1_' + str(image) + '.png', format='png', dpi=100, bbox_inches='tight')
	plt.savefig('pred3.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.close(fig)

        fig = plt.figure(figsize=(14,16))
	plt.ylim([ -2.5, 2.5 ])
        plt.plot(inputs1, vals1, color='y', marker='o', markersize=20, linestyle='None')
        plt.plot(pareto_set_locations, front[ :, 0 ], color='b', marker='x', markersize=20, linestyle='None')
	plt.plot(grid, val_f1, 'y', linestyle = '-', linewidth = 5.0)
        plt.plot(inputs1, vals1, color='y', marker='o', markersize=20, linestyle='None')
	plt.savefig('sample1.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.savefig('sample1.png', format='png', dpi=300, bbox_inches='tight')
	plt.close(fig)

	fig = plt.figure(figsize=(14,16))
	plt.xlim([-3,3])
	plt.ylim([-3,3])
	plt.plot(all_values[:,0],all_values[:,1], color='r', marker='x', markersize=20, linestyle='None')
	plt.savefig('pareto_frontier_1.1.pdf',format='pdf', dpi=300, bbox_inches='tight')
	plt.close(fig)

        fig = plt.figure(figsize=(14,16))
	plt.ylim([ -2.5, 2.5 ])
        plt.plot(inputs2, vals2, color='g', marker='o', markersize=20, linestyle='None')
        plt.plot(pareto_set_locations, front[ :, 1 ], color='b', marker='x', markersize=20, linestyle='None')
	plt.plot(grid, val_f2, 'g', linestyle = '-', linewidth = 5.0)
        plt.plot(inputs2, vals2, color='g', marker='o', markersize=20, linestyle='None')
	plt.savefig('sample2.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.savefig('sample2.png', format='png', dpi=300, bbox_inches='tight')
	plt.close(fig)

	fig = plt.figure(figsize=(14,16))
	plt.ylim([ -2.5, 2.5 ])
        plt.plot(inputs3, vals3, color='r', marker='o', markersize=20, linestyle='None')
        plt.plot(pareto_set_locations, cons[0](pareto_set_locations, False), color='b', marker='x', markersize=20, linestyle='None')
	plt.plot(grid, val_c1, 'r', linestyle = '-', linewidth = 5.0)
        plt.plot(inputs3, vals3, color='r', marker='o', markersize=20, linestyle='None')
	plt.savefig('sample3.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.savefig('sample3.png', format='png', dpi=300, bbox_inches='tight')
	plt.close(fig)
	# We now compute the conditional predictive distribution
	...
	pesm = PESMC(num_dims = input_space.num_dims, input_space = input_space, grid=grid, opt = options)

	ep_solutions_dict = dict()

	for i in range(n_mcmc_iters):
		objectives_dict['f1'].set_state(i)
		objectives_dict['f2'].set_state(i)
		constraints_dict['c1'].set_state(i)
		ep_solutions_dict[ str(i) ] = ep(objectives_dict, pareto_set_locations, minimize=True, con_models=constraints_dict)
	...
	for i in range(n_mcmc_iters):
		...
		objectives_dict['f2'].set_state(i)
		constraints_dict['c1'].set_state(i)
		result = predictEP_multiple_iter_optim(objectives_dict, constraints_dict, ep_solutions_dict[ str(i) ], pareto_set_locations, spacing,  \
				n_iters = 1, damping = .1, no_negatives = True, minimize = True)
			
		new_mean1 += result['mf']['f1']
		new_mean2 += result['mf']['f2']
		new_mean3 += result['mc']['c1']
		new_var1 += result['vf']['f1']
		new_var2 += result['vf']['f2']
		new_var3 += result['vc']['c1']

	new_mean1 /= float(n_mcmc_iters)
	new_mean2 /= float(n_mcmc_iters)
	new_mean3 /= float(n_mcmc_iters)
	new_var1 /= float(n_mcmc_iters)
	new_var2 /= float(n_mcmc_iters)
	new_var3 /= float(n_mcmc_iters)

        fig = plt.figure(figsize=(14,16))
	plt.ylim([ -1.4, 1.5 ])
        plt.plot(inputs1, vals1, color='y', marker='o', markersize=20, linestyle='None')
	plt.plot(spacing, new_mean1, 'y', linestyle='-', linewidth = 5.0)
	plt.plot(spacing, new_mean1 + np.sqrt(new_var1), color = 'y', linestyle = ':', linewidth = 4.0)
	plt.plot(spacing, new_mean1 - np.sqrt(new_var1), color = 'y', linestyle = ':', linewidth = 4.0)

	scale = np.linspace(0, 1.0, 30)

	for j in range(30 - 1):
		plt.fill_between(spacing.flatten(), (new_mean1 - np.sqrt(new_var1) * scale[ j ] + 1e-3), \
			(new_mean1 - np.sqrt(new_var1) * scale[ j + 1 ]), \
			linewidth = 0.0, color = (1.0, 1.0, 0.0, 0.5 * (1.0 - scale[ j ])))
		plt.fill_between(spacing.flatten(), (new_mean1 + np.sqrt(new_var1) * scale[ j ] - 1e-3), \
			(new_mean1 + np.sqrt(new_var1) * scale[ j + 1 ]), \
			linewidth = 0.0, color = (1.0, 1.0, 0.0, 0.5 * (1.0 - scale[ j ])))
        ...
        plt.plot(inputs1, vals1, color='y', marker='o', markersize=20, linestyle='None')
	plt.savefig('cond_pred1.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.savefig('cond_pred1.png', format='png', dpi=300, bbox_inches='tight')
	plt.close(fig)
 
        fig = plt.figure(figsize=(14,16))
	plt.ylim([ -1.5, 1.5 ])
        plt.plot(inputs2, vals2, color='g', marker='o', markersize=20, linestyle='None')
	plt.plot(spacing, new_mean2, 'g', linestyle='-', linewidth = 5.0)
	plt.plot(spacing, new_mean2 + np.sqrt(new_var2), color = 'g', linestyle = ':', linewidth = 4.0)
	plt.plot(spacing, new_mean2 - np.sqrt(new_var2), color = 'g', linestyle = ':', linewidth = 4.0)

	scale = np.linspace(0, 1.0, 30)

	for j in range(30 - 1):
		plt.fill_between(spacing.flatten(), (new_mean2 - np.sqrt(new_var2) * scale[ j ] + 1e-3), \
			(new_mean2 - np.sqrt(new_var2) * scale[ j + 1 ]), \
			linewidth = 0.0, color = (0.5, 1.0, 0.5, 0.5 * (1.0 - scale[ j ])))
		plt.fill_between(spacing.flatten(), (new_mean2 + np.sqrt(new_var2) * scale[ j ] - 1e-3), \
			(new_mean2 + np.sqrt(new_var2) * scale[ j + 1 ]), \
			linewidth = 0.0, color = (0.5, 1.0, 0.5, 0.5 * (1.0 - scale[ j ])))
        ...
        plt.plot(inputs2, vals2, color='g', marker='o', markersize=20, linestyle='None')
	plt.savefig('cond_pred2.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.savefig('cond_pred2.png', format='png', dpi=300, bbox_inches='tight')
	plt.close(fig)

	fig = plt.figure(figsize=(14,16))
	plt.ylim([ -1.4, 1.5 ])
        plt.plot(inputs3, vals3, color='r', marker='o', markersize=20, linestyle='None')
	plt.plot(spacing, new_mean3, 'r', linestyle='-', linewidth = 5.0)
	plt.plot(spacing, new_mean3 + np.sqrt(new_var3), color = 'r', linestyle = ':', linewidth = 4.0)
	plt.plot(spacing, new_mean3 - np.sqrt(new_var3), color = 'r', linestyle = ':', linewidth = 4.0)

	scale = np.linspace(0, 1.0, 30)

	for j in range(30 - 1):
		plt.fill_between(spacing.flatten(), (new_mean3 - np.sqrt(new_var3) * scale[ j ] + 1e-3), \
			(new_mean3 - np.sqrt(new_var3) * scale[ j + 1 ]), \
			linewidth = 0.0, color = (1.0, 0.5, 0.5, 0.5 * (1.0 - scale[ j ])))
		plt.fill_between(spacing.flatten(), (new_mean3 + np.sqrt(new_var3) * scale[ j ] - 1e-3), \
			(new_mean3 + np.sqrt(new_var3) * scale[ j + 1 ]), \
			linewidth = 0.0, color = (1.0, 0.5, 0.5, 0.5 * (1.0 - scale[ j ])))
        ...
        plt.plot(inputs3, vals3, color='r', marker='o', markersize=20, linestyle='None')
	plt.savefig('cond_pred3.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.savefig('cond_pred3.png', format='png', dpi=300, bbox_inches='tight')
	plt.close(fig)	
...
#		ep_tmp_sol = dict()
#		ep_tmp_sol[ str(0) ] = ep_solutions_dict[ str(i) ]
#		par_set_tmp = dict()
#		par_set_tmp[ str(0) ] = pareto_set_locations
#		opt = dict()
#		opt['pesm_not_constrain_predictions'] = False
		...
		if i == 0:
			acq_1 = pesm.acquisition(objectives_dict, constraints_dict, grid, None, False, minimize=True, tasks = task1)
			acq_2 = pesm.acquisition(objectives_dict, constraints_dict, grid, None, False, minimize=True, tasks = task2)
			acq_3 = pesm.acquisition(objectives_dict, constraints_dict, grid, None, False, minimize=True, tasks = task3)
#			acq_1 = evaluate_acquisition_function_given_EP_solution(objectives_dict, grid, ep_tmp_sol, 
#				par_set_tmp, opt = opt)['f1']
#			acq_2 = evaluate_acquisition_function_given_EP_solution(objectives_dict, grid, ep_tmp_sol, 
#				par_set_tmp, opt = opt)['f2']
		else:
#			acq_1 += evaluate_acquisition_function_given_EP_solution(objectives_dict, grid, ep_tmp_sol, 
#				par_set_tmp, opt = opt)['f1']
#			acq_2 += evaluate_acquisition_function_given_EP_solution(objectives_dict, grid, ep_tmp_sol, 
#				par_set_tmp, opt = opt)['f2']
	...
	acq_1 /= float(n_mcmc_iters)
	acq_2 /= float(n_mcmc_iters)
	acq_3 /= float(n_mcmc_iters)

        fig = plt.figure(figsize=(14,16))
	ax = fig.add_subplot(111)
	plt.ylim([ 0.0, 0.9 ])
	ymax = max(acq_1)
	xpos = np.where(acq_1==ymax)[0][0]
	xmax = grid[xpos]
	plt.plot(grid, acq_1, 'y', linestyle = '-', linewidth = 5.0)
	ax.annotate('Max', xy=(xmax, ymax), xytext=(xmax, ymax+0.05),
            arrowprops=dict(facecolor='black', shrink=0.05),
            )
	plt.fill_between(grid.flatten(), acq_1.flatten(), facecolor = '#F0F06C', edgecolor = 'yellow', linewidth = 3.0)
	plt.savefig('acq1.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.savefig('acq1.png', format='png', dpi=300, bbox_inches='tight')
	plt.close(fig)
 
        fig = plt.figure(figsize=(14,16))
	ax = fig.add_subplot(111)
	plt.ylim([ 0.0, 0.9 ])
	plt.plot(grid, acq_2*0.8, 'g', linestyle = '-', linewidth = 5.0)
	ymax = max(acq_2*0.8)
        xpos = np.where((acq_2*0.8)==ymax)[0][0]
        xmax = grid[xpos]
        ax.annotate('Max', xy=(xmax, ymax), xytext=(xmax, ymax+0.05),
            arrowprops=dict(facecolor='black', shrink=0.05),
            )
	plt.fill_between(grid.flatten(), acq_2.flatten()*0.8, facecolor = (0.5, 1.0, 0.5), edgecolor = 'green', linewidth = 3.0)
	plt.savefig('acq2.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.savefig('acq2.png', format='png', dpi=300, bbox_inches='tight')
	plt.close(fig)

	fig = plt.figure(figsize=(14,16))
	ax = fig.add_subplot(111)
	plt.ylim([ 0.0, 0.9 ])
	plt.plot(grid, acq_3, 'r', linestyle = '-', linewidth = 5.0)
	ymax = max(acq_3)
        xpos = np.where(acq_3==ymax)[0][0]
        xmax = grid[xpos]
        ax.annotate('Max', xy=(xmax, ymax), xytext=(xmax, ymax+0.05),
            arrowprops=dict(facecolor='black', shrink=0.05),
            )
	plt.fill_between(grid.flatten(), acq_3.flatten(), facecolor = '#FC5B5B', edgecolor = 'red', linewidth = 3.0)
	plt.savefig('acq3.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.savefig('acq3.png', format='png', dpi=300, bbox_inches='tight')
	plt.close(fig)	

        fig = plt.figure(figsize=(14,16))
	ax = fig.add_subplot(111)
	plt.ylim([ 0.0, 2.0 ])
	plt.plot(grid, (acq_1 + acq_2 + acq_3), 'b', linestyle = '-', linewidth = 5.0)
	ymax = max((acq_1 + acq_2 + acq_3))
        xpos = np.where(((acq_1 + acq_2 + acq_3))==ymax)[0][0]
        xmax = grid[xpos]
        ax.annotate('Max', xy=(xmax, ymax), xytext=(xmax, ymax+0.05),
            arrowprops=dict(facecolor='black', shrink=0.05),
            )
	plt.fill_between(grid.flatten(), (acq_1.flatten() + acq_2.flatten() + acq_3.flatten()), facecolor = (0.5, 0.5, 1.0), edgecolor = 'blue', linewidth = 3.0)
	plt.savefig('acq.pdf', format='pdf', dpi=300, bbox_inches='tight')
	plt.savefig('acq.png', format='png', dpi=300, bbox_inches='tight')
	plt.close(fig)
...
#		test_acquisition_function_decoupled(str(int(sys.argv[ 1 ]) + i))
	...
	generate_plot_illustration_of_the_process()
